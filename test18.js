...
(async () => {
  ...
  /* ================= 5. NO ZOMBIE BENCHES ================= */
  {
    ...
    // 'packed' is the case craft's own row — the blade has no such thing, so its
    // presence proves the live rubric belongs to the craft that is on the bench
    assert(live.defs.some(d => d.k === 'packed'), 'the live rubric is the new craft\'s, not the retired one\'s');
